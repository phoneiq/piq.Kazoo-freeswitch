...
static void extract_rtp_stats_simple(switch_core_session_t *session)
{
    ...
    if (stats) {
        switch_channel_set_variable_printf(channel, "rtp_in_packets", "%u", stats->inbound.packet_count);
        switch_channel_set_variable_printf(channel, "rtp_out_packets", "%u", stats->outbound.packet_count);
        switch_channel_set_variable_printf(channel, "rtp_mos_score", "%.2f", stats->inbound.mos);
        switch_channel_set_variable_printf(channel, "rtp_quality_pct", "%.2f", stats->inbound.R);
    }
}
	...
	if (!((globals.legs & CDR_LEG_A) && (globals.legs & CDR_LEG_B))) {
		if ((globals.legs & CDR_LEG_A)) {
			if (switch_channel_get_originator_caller_profile(channel)) {
				return SWITCH_STATUS_SUCCESS;
	/* Auto-extract RTP stats */
	extract_rtp_stats_simple(session);
			}
		} else {
