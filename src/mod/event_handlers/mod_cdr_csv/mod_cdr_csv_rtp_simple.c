...
static void extract_rtp_stats_simple(switch_core_session_t *session) {    switch_channel_t *channel = switch_core_session_get_channel(session);    switch_rtp_stats_t *stats = switch_core_media_get_stats(session, SWITCH_MEDIA_TYPE_AUDIO, NULL);    if (stats) {        switch_channel_set_variable_printf(channel, "rtp_in_packets", "%u", stats->inbound.packet_count);        switch_channel_set_variable_printf(channel, "rtp_out_packets", "%u", stats->outbound.packet_count);        switch_channel_set_variable_printf(channel, "rtp_mos_score", "%.2f", stats->inbound.mos);        switch_channel_set_variable_printf(channel, "rtp_quality_pct", "%.2f", stats->inbound.R);    }}
static switch_status_t my_on_reporting(switch_core_session_t *session)
{
	...
	if (globals.shutdown) {
t	extract_rtp_stats_simple(session);
