/*
 * mod_cdr_csv_rtp_rtp.c -- Enhanced CDR CSV Module with Automatic RTP Statistics
 * Based on original mod_cdr_csv_rtp.c
 * Enhanced by PhoneIQ Infrastructure Team - November 2025
 */
#include <switch.h>
#include <sys/stat.h>
...
SWITCH_MODULE_LOAD_FUNCTION(mod_cdr_csv_rtp_rtp_load);
SWITCH_MODULE_SHUTDOWN_FUNCTION(mod_cdr_csv_rtp_rtp_shutdown);
SWITCH_MODULE_DEFINITION(mod_cdr_csv_rtp_rtp, mod_cdr_csv_rtp_rtp_load, mod_cdr_csv_rtp_rtp_shutdown, NULL);

/* Enhanced: Automatic RTP Statistics Extraction */
static void extract_rtp_stats(switch_core_session_t *session)
{
    switch_channel_t *channel = switch_core_session_get_channel(session);
    switch_rtp_stats_t *audio_stats = switch_core_media_get_stats(session, SWITCH_MEDIA_TYPE_AUDIO, NULL);
    
    if (audio_stats) {
        switch_channel_set_variable_printf(channel, "rtp_audio_in_packet_count", "%u", audio_stats->inbound.packet_count);
        switch_channel_set_variable_printf(channel, "rtp_audio_out_packet_count", "%u", audio_stats->outbound.packet_count);
        switch_channel_set_variable_printf(channel, "rtp_audio_in_media_bytes", "%u", audio_stats->inbound.media_bytes);
        switch_channel_set_variable_printf(channel, "rtp_audio_out_media_bytes", "%u", audio_stats->outbound.media_bytes);
        switch_channel_set_variable_printf(channel, "rtp_audio_in_mos", "%.2f", audio_stats->inbound.mos);
        switch_channel_set_variable_printf(channel, "rtp_audio_in_quality_percentage", "%.2f", audio_stats->inbound.R);
        switch_channel_set_variable_printf(channel, "rtp_audio_in_jitter_loss_rate", "%.6f", audio_stats->inbound.lossrate);
    }
}
	cdr_leg_t legs;
} globals;

SWITCH_MODULE_LOAD_FUNCTION(mod_cdr_csv_rtp_load);
SWITCH_MODULE_SHUTDOWN_FUNCTION(mod_cdr_csv_rtp_shutdown);
SWITCH_MODULE_DEFINITION(mod_cdr_csv_rtp, mod_cdr_csv_rtp_load, mod_cdr_csv_rtp_shutdown, NULL);
...
static switch_status_t my_on_reporting(switch_core_session_t *session)
{
	...
	if (globals.shutdown) {
n	/* Auto-extract RTP statistics */
	extract_rtp_stats(session);
